...
if($gatewayName == 'Cart66Bitpay') {
  $total = Cart66Session::get('Cart66Cart')->getGrandTotal();
  $gateway = new Cart66Bitpay();
    ...
    if(count($errors) == 0 && !Cart66Session::get('Cart66InventoryWarning')) {
      ...
      if(count($errors) == 0) {
        ...
        if($transactionId) {
          ...
          try {
            throw new Cart66Exception(__('Your xxorder could not be completed for the following reasons:', 'cart66'), 66500);
          }
          catch(Cart66Exception $e) {
...
elseif($gatewayName == 'Cart66Bitpay') {
  $checkoutFormFile =  CART66_PATH . '/views/bitpay.php';
}
